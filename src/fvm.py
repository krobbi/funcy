from fvm_exit_code import ExitCode
...
class FVM:
    ...
    def load(self, bytecode: bytes) -> bool:
        ...
        if len(bytecode) < 16:
            ...
            return False
        elif int.from_bytes(bytecode[8:12], "little", signed=False) != self.FORMAT_VERSION:
            return False
    ...
    def step(self) -> None:
        ...
        if opcode == Opcode.HALT and self.validate_pop(1):
            ...
            arg_count: int = self.sm.pop()
            call_address: int = self.sm.pop()
            
            if not self.validate_pop(arg_count):
                return
    ...
    def crash(self) -> None:
        ...
        self.ec = ExitCode.FVM_CRASH.value
        self.ef = False
    
    
    def validate_fetch(self, amount: int) -> bool:
        """ Validate whether a fetch operation can be perfomed. """
        
        if self.ip >= 0 and self.ip + amount <= len(self.pm):
            return True
        else:
            self.crash()
            return False
    ...
    def fetch_int(self, size: int, is_signed: bool) -> int:
        ...
        value: int = int.from_bytes(self.pm[self.ip:self.ip + size], "little", signed=is_signed)
        self.ip += size
        return value
